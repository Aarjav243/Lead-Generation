...
- 🧠 **Optional AI enrichment** — `scripts/enrich.py` visits each lead's website (via [ScrapeGraphAI](https://github.com/ScrapeGraphAI/Scrapegraph-ai) + Gemini) to pull a contact name, direct email, services, and business-size signal.
- 💬 **Optional WhatsApp outreach** — `scripts/whatsapp-send.js` sends a message you write to every lead in a CSV, via your own WhatsApp Web session. `--dry-run` by default; resume-safe (skips already-sent numbers).
- 📊 **Optional local dashboard** — `scripts/dashboard-server.js` tracks total / no-website / messaged counts per department & city, reading straight off your CSVs (no database).
- 🔒 **Safe by default** — the scraper binds to `127.0.0.1` only, secrets and your real WhatsApp session are git-ignored, and the skill enforces rate-limit / legal / data-handling guardrails.
...
## Beyond scraping: enrich, outreach, dashboard

Three optional stages build on the raw scrape output:

```bash
# 1. Enrich leads that have a website (needs GOOGLE_API_KEY in .env — see .env.example)
pip install -r requirements.txt
python3 scripts/enrich.py results.csv --limit 20

# 2. Message leads via your own WhatsApp Web session (dry-run first, always)
node scripts/whatsapp-send.js --leads examples/leads.example.csv --message examples/message-template.example.txt
node scripts/whatsapp-send.js --leads your-leads.csv --message your-message.txt --live

# 3. Track progress across departments/cities in a local dashboard
cp dashboard-manifest.example.json dashboard-manifest.json   # then edit to point at your CSVs
node scripts/dashboard-server.js   # http://127.0.0.1:8090
```
...
> `--dry-run` first, include an opt-out ("reply STOP"), keep volumes low and delays high (the script
> defaults to 30–60s between sends), and don't use this for anything but genuine, relevant business
> outreach. Scraped phone numbers are personal data — the same GDPR/CCPA guidance in the section below
> applies here too.
...
├── requirements.txt        ← Python deps for scripts/enrich.py
├── package.json            ← Node deps for the WhatsApp/dashboard scripts
├── dashboard-manifest.example.json  ← template for tracking your own leads in the dashboard
├── .gitignore
├── scripts/
...
│   ├── leads.example.csv           ← sample leads CSV for whatsapp-send.js
│   └── message-template.example.txt ← sample outreach message with {column} placeholders
└── .claude/
