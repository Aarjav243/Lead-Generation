...
BASE = os.environ.get("SCRAPER_BASE_URL", "http://127.0.0.1:8080")
...
SOCIAL_RE = {
    ...
    "facebook": re.compile(
        r"https?://(?:www\.|m\.|web\.)?facebook\.com/([A-Za-z0-9_.\-]+)", re.I
    ),
    "linkedin": re.compile(
...
def main():
    ...
    ap.add_argument(
        "--keyword", dest="also", action="append", help="extra keyword (repeatable)"
    )
    ...
    ap.add_argument(
        "--max-time", type=int, default=600, help="job time limit in SECONDS"
    )
    ap.add_argument(
        "--out",
        default=None,
        help="output file path (default: results-<id>.csv). "
        "The extension picks the format: .csv (default) or .json",
    )
    ap.add_argument(
        "--json", action="store_true", help="write JSON instead of the default CSV"
    )
    ap.add_argument(
        "--full",
        action="store_true",
        help="keep ALL raw columns (default: lean lead fields)",
    )
    ap.add_argument(
        "--fields",
        help="comma-separated columns to keep (overrides the default lead set)",
    )
    ap.add_argument(
        "--socials",
        action="store_true",
    ...
    if not (lat and lon):
        ...
        if not coords:
            sys.exit(
                "✗ Could not resolve coordinates. Pass lat/lon or a clearer --city."
            )
        lat, lon = coords
        print(f"  → {lat}, {lon}")
    ...
    if a.email:
        print(
            "ℹ Email extraction is ON (visits each business website; a bit slower). "
    ...
    if a.depth >= 15 or len(keywords) >= 10:
        ...
        print(
            "    Running it back-to-back without proxies can get your IP temporarily rate-limited by"
        )
        print(
            "    Google (clears in minutes–hours; returns empty/failed jobs meanwhile). For big or"
        )
    ...
    try:
        ...
        sys.exit(
            f"✗ Scraper not reachable at {BASE} — run 'docker compose up -d' first.\n  ({e})"
        )
    ...
    print(
        f"▶ Creating job: {len(keywords)} keyword(s) @ {lat},{lon} depth={a.depth} email={a.email}"
    )
    for k in keywords:
        print(f"    • {k}")
    try:
    ...
    if a.socials:
        ...
        print(
            "  ℹ Cost: this runs in CODE = 0 LLM tokens. It only adds ~40–50 tokens per business"
        )
        print(
            "    IF you load the results into an AI's context (negligible if you keep the file on disk)."
        )
    ...
    for r in results[:5]:
        tail = (
            f" | IG:{r.get('instagram', '') or '—'}"
            if a.socials
            else f" | {r.get('website', '')}"
        )
        print(
            f"  • {r.get('title', '')} | {r.get('phone', '')} | {r.get('emails', '')}{tail}"
        )
