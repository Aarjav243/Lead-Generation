...
| **Python 3** | `scripts/scrape.py`; `scripts/enrich.py` (optional AI enrichment) | https://www.python.org/downloads/ |
| **Node.js** *(optional)* | only for `scripts/whatsapp-send.js` and `scripts/dashboard-server.js` | https://nodejs.org |
...
## 5. Optional: enrich, WhatsApp outreach, dashboard

```bash
# Enrich leads that have a website (needs GOOGLE_API_KEY in .env — a free Gemini key works)
pip install -r requirements.txt
python3 scripts/enrich.py results.csv --limit 20

# Message leads via your own WhatsApp Web session — always dry-run first
npm install
node scripts/whatsapp-send.js --leads examples/leads.example.csv --message examples/message-template.example.txt
node scripts/whatsapp-send.js --leads your-leads.csv --message your-message.txt --live   # scans a QR code on first run

# Track total / no-website / messaged counts per department & city
cp dashboard-manifest.example.json dashboard-manifest.json   # edit to point at your CSVs
node scripts/dashboard-server.js   # http://127.0.0.1:8090
```

Your message file can use `{column}` placeholders matching your leads CSV's headers (e.g. `{title}`).
See the **[README's "Beyond scraping" section](README.md#beyond-scraping-enrich-outreach-dashboard)**
for the responsible-use warning before sending anything `--live`.
